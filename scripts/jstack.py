...
class JStack(object):
    ...
    def parse(self):
        ...
        #re_thread = re.compile(r'^"([^\"]+)"\s+\#[0-9]+\s+(daemon )?prio=([0-9]+) tid=(0x[0-9a-f]+) nid=0x[0-9a-f]+ ([^ ])')
        re_thread =re.compile(r'^"([^\"]+)"\s+\#[0-9]+\s+(daemon )?prio=([0-9]+) os_prio=([0-9]+) tid=(0x[0-9a-f]+) nid=(0x[0-9a-f]+) ([\d\D]+)')
        (en_thread.SOCK, en_thread.DAEMON, en_thread.PRIO, en_thread.OSPRIO,
         en_thread.TID, en_thread.NID, en_thread.WCHAN) = range(1, 8)
        ...
        for line in reader:
            ...
            if m_thread:
                ...
                thread = {'sock': m_thread.group(en_thread.SOCK),
                          'id': int(m_thread.group(en_thread.NID),16),
